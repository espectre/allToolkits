...
import image_helper
import numpy as np
# the script to append bbox to pascal voc xml file
# 定义一个常量字典
PASCAL_VOC = {
    'folder': 'VOC2007',
    'source': {
...
def createXmlFileByLabelXJsonList(labelxJsonLine=None, basePath=None):
    ...
    if res == None:
        ...
        errorImage_path_name = os.path.join(
            errorImageDir, imageLocalImagePath.split('/')[-1])
...
def main():
    jsonlistFile = ""
    datasetBasePath = ""
    convertLabelxJsonListToXmlFile(
        jsonlistFile=jsonlistFile, datasetBasePath=datasetBasePath)
