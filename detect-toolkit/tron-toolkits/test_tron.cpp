...
int main(int argc, char const *argv[])
{
    ...
    std::ifstream input(im_file);
    std::vector<std::string> test_image_list;
    for(std::string line;getline(input,line);){
        test_image_list.push_back(line);
    }
    /*
    for(int i=0;i<test_image_list.size();i++){
        std::cout<<test_image_list[i]<<std::endl;
    }
    */
    Tron::Timer timer;
    double time_cost = 0;
    ...
    {
        for (int i = 0; i < test_image_list.size(); i++){
            std::string i_image_file = test_image_list[i];
            std::cout << i_image_file << std::endl; 
            time_cost = 0;
            for (int i = 0; i < loop_count; ++i)
            {
                // Read image from file to bytes
                auto *im_fp = fopen(i_image_file.c_str(), "rb");
