...
# 输入日期进行日常推理
# 要判断是否发生中断
# eg : nohup ./Daily_Inference.sh 20180801 & 
set -x
...
# process log file and result save in the folder :  $save_path/Inputdata/***
save_path="/workspace/data/BK/logFiles"

goto_step()
{
    # status == 0 , just create status.log file
    if [ $1 -eq 0 ]; then
        echo "1" >$Filename
    # status == 1 , create status.log finish , begin download log file
    elif [ $1 -eq 1 ]; then
        echo "Download data:"
