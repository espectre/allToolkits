...
def merge_conv_and_bn(net, i_conv, i_bn, i_scale):
    ...
    alpha = scale_weight / \
        np.sqrt(bn_variance / num_bn_samples[0] + np.finfo(np.double).eps)
    net.params[key_conv][1].data[:] = bias * alpha + \
        (scale_bias - (bn_mean / num_bn_samples[0]) * alpha)
...
def set_engine_caffe(layer, net, model, i):
    if layer.type == 'Convolution':
        if layer.convolution_param.kernel_size == 1\
            ...
            layer.convolution_param.engine = dict(
                layer.convolution_param.Engine.items())['CAFFE']
...
def main(args):
    ...
    net = load_and_fill_biases(
        args.model, args.weights, args.model + '.temp.pt', None)
