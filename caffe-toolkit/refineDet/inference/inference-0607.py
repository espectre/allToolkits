...
def post_eval(input_image, batch_image_path, batch_image_h_w, output, reqs=None):
    ...
    the_bathc_image_cls_result_list = []
    for i in range(cur_batchsize):
        batch_index_result = output['prob'][i]
        label_cls, score_cls = cls_postprocess(batch_index_result)
        # print(label_cls)
        # print(score_cls)
        the_bathc_image_cls_result_list.append([label_cls, score_cls])
    ...
    for image_id in range(cur_batchsize):
        cls_result = the_bathc_image_cls_result_list[image_id]
        if image_id in image_result_dict.keys():
        ...
        resps.append(
            {"code": 0, "message": batch_image_path[image_id], "result": result, "cls_result": cls_result[0]})
    _t2 = time.time()
...
def process_res(res_list=None):
    for image_res in res_list:
        cls_index = image_res['cls_result']
        # image_res is dict
        bbox_list = image_res['result']['detections']
        if args.visFlag == 1:
            vis(imageName=image_res['message'],
                bbox_list=bbox_list, cls_index=cls_index)
