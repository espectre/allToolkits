# -*- coding:utf-8 -*-
"""
    the script to inference : caffe model
"""
import numpy as np
import sys
import os
import cv2
# add used caffe path to system path
sys.path.insert(
...
import urllib
import random
import inference_demo_config as CONFIG_FILE
def parser_args():
    parser = ArgumentParser('inference :::::')
    parser.add_argument('--inputFile', dest='inputFile', help='image list file : url or local imgage abs path',
                        default=None, type=str,required=True)
    parser.add_argument('--inputFileFlag', dest='inputFileFlag', help='0:local image file absolute path, 1:url',
                        default=0, type=int, required=True)
    # outputFileFlag : 1 regression format file，2 labelx json list format file
